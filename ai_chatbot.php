...
if ($userId) {
    ...
    if (strpos($userMessage, 'my points') !== false || strpos($userMessage, 'how many points') !== false) {
        ...
        $reply = "You currently have " . ($user['points'] ?? 0) . " points.";
    }
    // Count total vouchers
    elseif (strpos($userMessage, 'total voucher') !== false) {
        $stmt = $conn->prepare("SELECT COUNT(*) as total FROM voucher");
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $reply = "You have " . ($row['total'] ?? 0) . " available vouchers.";
    }
    // Last redeemed voucher
    elseif (strpos($userMessage, 'last voucher') !== false || strpos($userMessage, 'recent voucher') !== false) {
        ...
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $reply = "Your last redeemed voucher was '" . $row['title'] . 
